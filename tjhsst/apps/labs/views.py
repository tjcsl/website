...
def index(request):
    ...
    return render(
        ...
        {
            "labs": [
                (lab, itertools.zip_longest(lab.prerequisites.all(), lab.recommended.all()))
                for lab in labs
            ],
...
def show_courses(request):
    ...
    if "submit" in request.GET:
        labs = Lab.objects.filter(
            Q(prerequisites__url__in=course_urls) | Q(recommended__url__in=course_urls)
        ).distinct()
        ...
        return render(
            ...
            {
                "all_courses": sorted(
                    Course.objects.all(), key=lambda course: course.nickname_or_name
                ),
                "course_urls": course_urls,
            },
        )
...
def edit(request, lab_url):
    lab = get_object_or_404(Lab, url=lab_url)
    if request.user.is_superuser or request.user in lab.admins.all():
        ...
        return render(request, "labs/edit.html", {"lab": lab, "lab_form": form})
    else:
        raise Http404
